...
export class TaskPage {
  ...
  starTaskAndVerify = async ({ taskName }: TaskName) => {
    ...
    await expect(
      this.page.getByTestId(TASKS_TABLE_SELECTORS.pendingTasksTable).getByRole('row').nth(1) // Using nth methods here since we want to verify the first row of the table
