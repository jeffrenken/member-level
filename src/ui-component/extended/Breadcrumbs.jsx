...
// material-ui
import { Box, Card, Divider, Grid, Breadcrumbs as MuiBreadcrumbs, Typography } from '@mui/material';
...
const Breadcrumbs = ({ card, divider, icon, icons, maxItems, navigation, rightAlign, separator, title, titleBottom, ...others }) => {
  ...
  const separatorIcon = 's'; //separator ? <SeparatorIcon stroke={1.5} size="1rem" /> : <IconTallymark1 stroke={1.5} size="1rem" />;
  ...
  if (main && main.type === 'collapse') {
    CollapseIcon = main.icon ? main.icon : 'ICONFIle';
    mainContent = (
  ...
  if (item && item.type === 'item') {
    ...
    ItemIcon = item.icon ? item.icon : 'F';
    itemContent = (
      <Typography
        variant="subtitle1"
        sx={{
